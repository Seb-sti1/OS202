...
std::vector<int>
computeMandelbrotSet(int W, int H, int localH, int offsetH, int maxIter, bool clock)
{
    std::chrono::time_point<std::chrono::system_clock> start, end;

    std::vector<int> pixels(W * localH);
    ...
    {
        computeMandelbrotSetRow(W, H, maxIter, offsetH + i, pixels.data() + W * (localH - i - 1));
    }
...
int main(int argc, char *argv[])
{
    ...
    if (rank == 0)
    {
        std::cout << "Mandelbrot without MPI" << std::endl;

        auto no_mpi = computeMandelbrotSet(W, H, H, 0, maxIter, true);

        savePicture("mandelbrot_no_mpi.tga", W, H, no_mpi, maxIter);
    }
    ...
    std::vector<int> finalIters;

    if (rank == 0)
    {
        std::cout << "Mandelbrot with MPI" << std::endl;

        start = std::chrono::system_clock::now();
        finalIters.resize(W * H);
        globalData.resize(nbp);
        for (int i = 0; i < nbp; ++i)
        {
            globalData[nbp - i - 1] = i * localH;
        }
    }

    MPI_Scatter(globalData.data(), 1, MPI_INT, offsetH.data(), 1, MPI_INT, 0, global);

    std::cout << "[" << rank << "] : localH " << localH << ", offset " << offsetH.at(0) << std::endl;

    auto iters = computeMandelbrotSet(W, H, localH, offsetH.at(0), maxIter, false);

    MPI_Gather(iters.data(), localH * W, MPI_INT, finalIters.data(), localH * W, MPI_INT, 0, global);
    ...
    if (rank == 0)
    {
        ...
        std::cout << "Temps calcul ensemble mandelbrot : " << elapsed_seconds.count()
                  << std::endl;
        std::cout << "Image is " << W << "x" << finalIters.size() / W << std::endl;
        savePicture("mandelbrot_mpi.tga", W, H, finalIters, maxIter);
    }
